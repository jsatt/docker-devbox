...
RUN apt-get update \
    ...
    && apt-get install -y \
       ...
       sudo whois man-db apt-transport-https ca-certificates curl gnupg-agent \
       software-properties-common unminimize \
       && yes | unminimize \
...
RUN set -eux \
    ...
    && apt-get install -y \
       vim fuse git zsh silversearcher-ag tmux docker-ce-cli htop libxml2-utils rsync fd-find ripgrep \
